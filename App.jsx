...
// Final 25/26 order — confirmed facts: Larne champions (3rd title), Glentoran runner-up,
// Coleraine + Linfield also qualified for Europe (Linfield via the 3rd-7th play-off, beating
// Dungannon Swifts in the final), Bangor promoted this season, Crusaders in the relegation
// play-off, Glenavon relegated bottom. Positions 6-10 and exact points are provisional —
// correct these once you've got the real final table in front of you.
const TABLE = [
  { club: "LAR", p: 38, w: 26, d: 7, l: 5, gd: 44, pts: 85, note: "C", form: ["W","W","D","W","W"] },
  { club: "COL", p: 38, w: 24, d: 8, l: 6, gd: 34, pts: 80, note: "IC", form: ["W","D","W","W","L"] },
  { club: "GLE", p: 38, w: 23, d: 9, l: 6, gd: 31, pts: 78, note: "E", form: ["W","W","L","W","D"] },
  { club: "LIN", p: 38, w: 20, d: 8, l: 10, gd: 22, pts: 68, note: "EPO", form: ["D","W","W","L","W"] },
  { club: "DUN", p: 38, w: 15, d: 9, l: 14, gd: 2, pts: 54, note: "", form: ["W","L","D","L","W"] },
  { club: "CLI", p: 38, w: 14, d: 8, l: 16, gd: -6, pts: 50, note: "", form: ["L","W","D","W","L"] },
  { club: "CAR", p: 38, w: 12, d: 10, l: 16, gd: -10, pts: 46, note: "", form: ["D","L","W","D","L"] },
  { club: "POR", p: 38, w: 11, d: 9, l: 18, gd: -14, pts: 42, note: "", form: ["L","D","L","W","D"] },
  { club: "BAN", p: 38, w: 10, d: 10, l: 18, gd: -18, pts: 40, note: "P", form: ["D","W","L","L","D"] },
  { club: "BAL", p: 38, w: 9, d: 9, l: 20, gd: -22, pts: 36, note: "", form: ["L","D","L","W","L"] },
  { club: "CRU", p: 38, w: 7, d: 9, l: 22, gd: -30, pts: 30, note: "PO", form: ["L","D","L","L","W"] },
  { club: "GLV", p: 38, w: 6, d: 7, l: 25, gd: -41, pts: 25, note: "R", form: ["L","L","L","D","L"] },
];
...
const RECORDS = [
  ...
  { big: "56", label: "Linfield league titles", sub: "More top-flight titles than any other club in the league's history" },
...
const PREDICTOR_GW = {
  ...
  fixtures: [
    { id: "f1", home: { external: "Tre Fiori" }, away: { club: "LAR" }, comp: "UCL Q1 · Tue", result: null },
    { id: "f2", home: { club: "GLE" }, away: { external: "RFS" }, comp: "UECL Q1 · Thu", result: null },
    { id: "f3", home: { external: "Nõmme Kalju" }, away: { club: "LIN" }, comp: "UECL Q1 · Thu", result: null },
  ],
};
...
// ===== Kit Tracker 26/27 =====
// status: "revealed" (with date + note) or "waiting". Jerseys are GIBSON illustrations
// in traditional club colours — not replicas of actual designs.
const KITS = [
  { club: "GLE", status: "revealed", date: "18 Jun", note: "Home kit unveiled — the club says it's inspired by one of the most iconic shirts the Glens have ever worn." },
  { club: "LAR", status: "waiting" },
  { club: "LIN", status: "waiting" },
  { club: "COL", status: "waiting" },
  { club: "CLI", status: "waiting" },
  { club: "CRU", status: "waiting" },
  { club: "DUN", status: "waiting" },
  { club: "BAL", status: "waiting" },
  { club: "POR", status: "waiting" },
  { club: "BAN", status: "waiting" },
  { club: "CAR", status: "waiting" },
  { club: "GLV", status: "waiting" },
];

// Josh's live Ko-fi page
const KOFI_URL = "https://ko-fi.com/gibson575";
...
const EURO = [
  {
    ...
    opp: "Tre Fiori", oppCountry: "San Marino",
    legs: [
    ...
    note: "Larne play European home games at Windsor Park. Even defeat isn't the end — the loser drops into Conference League Q3.",
  },
  {
    club: "GLE", comp: "Conference League", round: "First qualifying round",
    opp: "RFS", oppCountry: "Latvia",
    legs: [
  ...
  },
  {
    ...
    opp: "Nõmme Kalju", oppCountry: "Estonia",
    legs: [
  ...
  },
  {
    ...
    opp: "HJK Helsinki", oppCountry: "Finland",
    legs: [
...
/* ================= VIEWS ================= */
function TableView() {
  const noteLabel = { C: "Champions · Gibson Cup", IC: "Irish Cup winners · Europe", E: "Europe (automatic)", EPO: "Europe (via play-off)", P: "Promoted this season", PO: "Relegation play-off", R: "Relegated" };
  const noteColor = { C: "#3DDC84", IC: "#5EC8F2", E: "#FFB627", EPO: "#5EC8F2", P: "#8FA69B", PO: "#E0A252", R: "#E05252" };
  return (
    <div style={{ animation: "riseIn 0.4s ease-out" }}>
      ...
      <div style={{ display: "grid", gap: 6 }}>
        {TABLE.map((row, i) => (
          <div key={row.club} style={{
            ...
            borderLeft: `3px solid ${row.note ? noteColor[row.note] : "transparent"}`,
            ...
            <div style={{
              ...
              color: row.note ? noteColor[row.note] : dim, width: 22, textAlign: "center",
              fontVariantNumeric: "tabular-nums", flexShrink: 0,
            }}>{i + 1}</div>
            <Crest club={row.club} size={26} />
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontWeight: 700, fontSize: 14.5, color: chalk, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                {CLUBS[row.club].name}
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 3 }}>
                <span style={{ fontSize: 10.5, color: dim, fontVariantNumeric: "tabular-nums" }}>
                  P{row.p} · {row.gd > 0 ? "+" : ""}{row.gd} GD
                </span>
                <span style={{ display: "flex", gap: 3 }}>
                  {row.form.map((f, j) => (
                    <span key={j} style={{
                      width: 6.5, height: 6.5, borderRadius: "50%", background: formColor(f),
                      opacity: j === row.form.length - 1 ? 1 : 0.55,
                    }} />
                  ))}
                </span>
              </div>
              {row.note && <div style={{ fontSize: 9.5, color: noteColor[row.note], marginTop: 3 }}>{noteLabel[row.note]}</div>}
            </div>
      ...
      <div style={{ fontSize: 10, color: dim, marginTop: 8, lineHeight: 1.5 }}>
        Champions, European places, promotion and relegation confirmed. Full points archive import in progress —
        spotted an error? Corrections welcome via the Support page.
      </div>
    </div>
  );
}
...
function FixturesView() {
  ...
  const fixtures = CLUB_FIXTURES[club];
  const c = CLUBS[club];
  return (
    <div style={{ animation: "riseIn 0.4s ease-out" }}>
      <div style={{ fontSize: 10, color: dim, letterSpacing: "0.14em", textTransform: "uppercase", marginBottom: 8 }}>Next up · pick your club</div>
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 14 }}>
        {Object.keys(CLUBS).map((k) => (
      ...
      {fixtures ? (
        <div style={{ border: `1px solid ${faint}`, borderRadius: 14, overflow: "hidden" }}>
          {fixtures.map((f, i) => (
            <div key={i} style={{
              ...
              borderBottom: i < fixtures.length - 1 ? `1px solid ${faint}` : "none",
              opacity: f.opp.includes("*") ? 0.6 : 1,
            }}>
              <div style={{ fontFamily: "'Barlow Condensed'", fontWeight: 800, fontSize: 14, color: "#FFB627", width: 74, flexShrink: 0, lineHeight: 1.2 }}>{f.date}</div>
        ...
        <div style={{ border: `1px dashed ${faint}`, borderRadius: 14, padding: "24px 16px", textAlign: "center" }}>
          <div style={{ fontSize: 26, marginBottom: 8 }}>🗓️</div>
          <div style={{ fontSize: 13.5, fontWeight: 600, color: chalk, marginBottom: 4 }}>No European football this summer</div>
          <div style={{ fontSize: 12, color: dim, lineHeight: 1.5 }}>
            The 26/27 Premiership fixture list hasn't been released yet — the league kicks off in August.
            {c.name}'s opening five games will appear here the day they're announced.
          </div>
        </div>
      )}
    </div>
...
function Jersey({ club, size = 74, dimmed = false }) {
  const c = CLUBS[club];
  const [c1, c2] = c.colors;
  return (
    <svg width={size} height={size} viewBox="0 0 100 100" style={{ opacity: dimmed ? 0.45 : 1 }} aria-label={c.name + " jersey"}>
      <defs>
        <clipPath id={`shirt-${club}`}>
          <path d="M35 12 L20 20 L8 38 L20 46 L24 38 V88 H76 V38 L80 46 L92 38 L80 20 L65 12 C62 20 55 24 50 24 C45 24 38 20 35 12 Z" />
        </clipPath>
      </defs>
      <path d="M35 12 L20 20 L8 38 L20 46 L24 38 V88 H76 V38 L80 46 L92 38 L80 20 L65 12 C62 20 55 24 50 24 C45 24 38 20 35 12 Z"
        fill={c1} stroke="rgba(237,245,239,0.3)" strokeWidth="2" />
      <g clipPath={`url(#shirt-${club})`}>
        {c.pattern === "stripes" && [18, 34, 50, 66, 82].map((x) => <rect key={x} x={x - 4} y="0" width="8" height="100" fill={c2} />)}
        {c.pattern === "sleeve" && <><path d="M8 38 L20 20 L35 12 L38 22 L24 38 L20 46 Z" fill={c2} /><path d="M92 38 L80 20 L65 12 L62 22 L76 38 L80 46 Z" fill={c2} /></>}
        {c.pattern === "plain" && <rect x="0" y="52" width="100" height="6" fill={c2} />}
      </g>
      <path d="M50 24 C45 24 38 20 35 12 L42 10 C44 15 47 17 50 17 C53 17 56 15 58 10 L65 12 C62 20 55 24 50 24 Z" fill={c2} />
    </svg>
  );
}

function KitsView() {
  const revealed = KITS.filter((k) => k.status === "revealed").length;
  return (
    <div style={{ animation: "riseIn 0.4s ease-out" }}>
      <div style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
          <div>
            ...
            <div style={{ fontSize: 11.5, color: dim, marginTop: 4 }}>
              New league match ball revealed 29 Jun · kit season is on
            </div>
          </div>
          <div style={{ textAlign: "center" }}>
            <div style={{ fontFamily: "'Barlow Condensed'", fontWeight: 800, fontSize: 28, color: "#FFB627", lineHeight: 1, fontVariantNumeric: "tabular-nums" }}>{revealed}/12</div>
            <div style={{ fontSize: 9, color: dim, letterSpacing: "0.1em", textTransform: "uppercase" }}>Revealed</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(150px, 1fr))", gap: 10 }}>
        {KITS.map((k, i) => {
          const c = CLUBS[k.club];
          const isRevealed = k.status === "revealed";
          return (
            <div key={k.club} style={{
              background: isRevealed ? "rgba(61,220,132,0.05)" : "rgba(240,255,245,0.03)",
              border: `1px solid ${isRevealed ? "rgba(61,220,132,0.4)" : faint}`,
              borderRadius: 14, padding: "14px 10px", textAlign: "center",
              animation: `riseIn 0.4s ease-out ${i * 0.04}s backwards`,
            }}>
              <Jersey club={k.club} dimmed={!isRevealed} />
              <div style={{ fontSize: 12.5, fontWeight: 700, color: chalk, marginTop: 6 }}>{c.name}</div>
              {isRevealed ? (
                <div style={{
                  display: "inline-block", marginTop: 6, fontSize: 9, fontWeight: 800,
                  letterSpacing: "0.08em", textTransform: "uppercase", color: "#0B1512",
                  background: "#3DDC84", borderRadius: 999, padding: "3px 10px",
                }}>Revealed · {k.date}</div>
              ) : (
                <div style={{
                  display: "inline-block", marginTop: 6, fontSize: 9, fontWeight: 700,
                  letterSpacing: "0.08em", textTransform: "uppercase", color: dim,
                  border: `1px solid ${faint}`, borderRadius: 999, padding: "3px 10px",
                }}>Awaiting reveal</div>
              )}
              {k.note && <div style={{ fontSize: 10.5, color: dim, marginTop: 7, lineHeight: 1.45, textAlign: "left" }}>{k.note}</div>}
            </div>
          );
        })}
      </div>
      <div style={{ fontSize: 10, color: dim, marginTop: 12, lineHeight: 1.5 }}>
        Jerseys shown are GIBSON illustrations in traditional club colours, not the actual 26/27 designs —
        head to each club's official store for the real thing.
      </div>
    </div>
  );
}
