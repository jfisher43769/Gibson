...
// ground/capacity/seated were entered by hand from Wikipedia (2025–26 NIFL Premiership
// season page, retrieved 23 July 2026). lat/lon were supplied by the owner from Google
// Maps on 29 July 2026 and are what switches TRAVEL on: computeTravel() nulls EVERY club's
// total if even one opponent lacks coordinates, so all twelve had to land together.
// founded/website remain null until scripts/fetch-wikidata.js runs with network access.
// Re-running that script is now safe — it merges rather than overwrites, so these
// hand-entered and owner-verified values survive it (fixed alongside PR #23).
